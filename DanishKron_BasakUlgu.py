...
def zeroreplacement(in_list):
    res = in_list[:]
    for k,v in enumerate (in_list):
        if v == 0:
            res[k] = (in_list[k-1] + in_list[k+1]) / 2.0
    return res
...
rms = sqrt(mean_squared_error(zeroreplacement(test.VALUE), y_hat.naive))
print("RMSE: ",rms)
...
rms = sqrt(mean_squared_error(zeroreplacement(test.VALUE), y_hat_avg.avg_forecast))
print("RMSE: ",rms)
...
rms = sqrt(mean_squared_error(zeroreplacement(test.VALUE), y_hat_avg.moving_avg_forecast))
print("RMSE: ",rms)
...
rms = sqrt(mean_squared_error(zeroreplacement(test.VALUE), y_hat_avg.SES))
print("RMSE: ",rms)
...
rms = sqrt(mean_squared_error(zeroreplacement(test.VALUE), y_hat_avg.Holt_linear))
print("RMSE: ",rms)
...
rms = sqrt(mean_squared_error(zeroreplacement(test.VALUE), y_hat_avg.Holt_Winter))
print("RMSE: ",rms)
